...
def predict_top3_time(model, X, raceid):
    ...
    df_top3['Top 3 pred'] = False
    df_group = df_top3.groupby('raceid')
    for group in df_group.groups:
        df_top3.loc[top3_time(df_group.get_group(group)['Time']), 'Top 3 pred'] = True
    return df_top3
...
def predict_top3_prob(model, X, raceid):
    ...
    df_group = df_top3.groupby('raceid')
    for group in df_group.groups:
        df_top3.loc[top3_prob(df_group.get_group(group)['Top 3 prob']), 'Top 3 pred'] = True
    return df_top3
